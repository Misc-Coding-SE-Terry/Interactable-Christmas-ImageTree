...
const photoFiles = ['./assets/image1.JPG', './assets/image2.JPG', './assets/image3.JPG', './assets/image4.PNG', './assets/image5.PNG'];
const photoTextures = [];
...
function createCustomTexture(type) {
    ...
    if (type === 'gold_glow') {
        ...
        const grd = ctx.createRadialGradient(cx, cy, 0, cx, cy, 40);
        grd.addColorStop(0, '#FFFFFF'); 
        grd.addColorStop(0.2, '#90EE90'); 
        grd.addColorStop(0.5, '#32CD32');
        grd.addColorStop(1, 'rgba(0,0,0,0)');
        ctx.fillStyle = grd; ctx.fillRect(0,0,128,128);
...
const textures = {
    ...
    green: createCustomTexture('green_glow'),
...
let groupGreen, groupRed, groupGift; 
let photoMeshes = [];    
...
function init3D() {
    ...
    groupGreen = createParticleSystem('green', CONFIG.goldCount, 2.0);
...
function createParticleSystem(type, count, size) {
    ...
    else if(type === 'green') baseColor.setHex(0x32CD32);
...
function updateParticleGroup(group, type, targetState, speed, handRotY, time) {
    ...
    if (targetState === 'TREE') {
        ...
        for(let i=0; i<count; i++) {
            sizes[i] = baseSize;
            let brightness = 1.0;
            if(type === 'gold' || type === 'green' || type === 'red') {
...
function animate() {
    ...
    if (state === 'TREE') {
        ...
        loveMesh.scale.set(s,s,1);
                ...
                const baseAngle = groupGreen.rotation.y;
